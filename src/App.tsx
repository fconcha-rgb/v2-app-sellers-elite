...
/* ──────────────────────────────────────────────────────────────
  MOCK API (reemplaza con tus imports reales)
────────────────────────────────────────────────────────────── */
const delay = (ms: number) => new Promise<void>((r) => setTimeout(r, ms));

const MOCK_PROSPECTS: any[] = [];
const MOCK_SELLERS: any[] = [];
const MOCK_CUPOS: any[] = [];

const fetchProspects = async (): Promise<{ data: any[] }> => ({ data: MOCK_PROSPECTS });
const fetchSellers = async (): Promise<{ data: any[] }> => ({ data: MOCK_SELLERS });
const fetchCupos = async (): Promise<{ data: any[] }> => ({ data: MOCK_CUPOS });

const upsertProspect = async (_d: any): Promise<{ error: null | { message: string } }> => ({ error: null });
const deleteProspectDB = async (_id: string): Promise<{ error: null | { message: string } }> => ({ error: null });
const updateProspectStatus = async (_id: string, _st: string): Promise<{ error: null | { message: string } }> => ({ error: null });

const upsertSeller = async (_d: any): Promise<{ error: null | { message: string } }> => ({ error: null });
const deleteSellerDB = async (_sid: string): Promise<{ error: null | { message: string } }> => ({ error: null });

const upsertCupo = async (_d: any): Promise<{ error: null | { message: string } }> => ({ error: null });

/* ──────────────────────────────────────────────────────────────
  Constantes + Tipos base
────────────────────────────────────────────────────────────── */
const CATEGORIAS = ['Electro', 'Muebles/Hogar', 'Cat Dig', 'Moda', 'Belleza/Calzado'] as const;
const STAGES = ['Prospectos', 'Contactados', 'Interesados', 'No Interesado', 'Cerrados'] as const;
const ACTIVE_STAGES = ['Prospectos', 'Contactados', 'Interesados'] as const;
const MONTHS_SHORT = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'] as const;
const PLAN_TYPES = ['Full', 'Premium', 'Basico'] as const;
...
const PLAN_COLORS_LIGHT: Record<PlanType, string> = {
  Full: '#86EFAC',
  Premium: '#C4B5FD',
  Basico: '#7DD3FC',
};
...
const planC = (p: PlanType) => PLAN_COLORS[p] || C.secondary;
...
const mapProspect = (r: any): Prospect => ({
  id: String(r.id ?? ''),
  s: String(r.seller ?? ''),
  st: (r.status as Stage) ?? 'Prospectos',
  t: String(r.tipo ?? ''),
...
const mapSeller = (r: any): Seller => {
  ...
  if (r.custom_dctos) {
    try {
      const parsed = typeof r.custom_dctos === 'string' ? JSON.parse(r.custom_dctos) : r.custom_dctos;
...
const sortData = <T extends Record<string, any>>(data: T[], config: SortConfig): T[] =>
  data.slice().sort((a, b) => {
    let va = a[config.key];
    let vb = b[config.key];
...
const Pill = (props: { color: string; children: React.ReactNode }) => (
  <span
    style={{
      padding: '3px 10px',
      borderRadius: 20,
...
const ViewToggle = (props: { mode: 'monthly' | 'ytd'; onChange: (v: 'monthly' | 'ytd') => void }) => (
  <div style={{ display: 'flex', gap: 2, background: C.bgDark, padding: 2, borderRadius: 8 }}>
    {[
      ...
      <button
        key={k}
        onClick={() => props.onChange(k as 'monthly' | 'ytd')}
        style={{
          padding: '5px 12px',
          borderRadius: 6,
          fontSize: 11,
...
.collapse-btn{cursor:pointer;user-select:none;display:inline-flex;align-items:center;gap:4px;font-size:11px;font-weight:700;padding:2px 8px;border-radius:4px;transition:background .15s} 
.collapse-btn:hover{background:rgba(0,0,0,.05)}`;

/* ──────────────────────────────────────────────────────────────
  Tipos derivados para dashboard
────────────────────────────────────────────────────────────── */
type MonthlyRow = { name: MonthShort; idx: number } & Record<PlanType, number> & { total: number };

type KPI = {
  tot: number;
  act: number;
  planCounts: Record<PlanType, number>;
  planRevs: Record<PlanType, number>;
  pausa: number;
  fug: number;
  pipe: number;
  cerr: number;
...
export default function App() {
  const [tab, setTab] = useState<'dashboard' | 'hunting' | 'sellers'>('dashboard');
  ...
  const [fSt, setFSt] = useState<'Todos' | Stage>('Todos');
  const [q, setQ] = useState('');

  const [selS, setSelS] = useState<Seller | null>(null);

  // form dinámico (u0, d0, etc.)
  ...
  const [sPlanF, setSPlanF] = useState<'Todos' | PlanType>('Todos');
  const [sQ, setSQ] = useState('');

  const [dashView, setDashView] = useState<'monthly' | 'ytd'>('monthly');

  // Collapsible state for dashboard detail table
  const [expandedCats, setExpandedCats] = useState<Partial<Record<Categoria, boolean>>>({});

  const toggleCat = useCallback((cat: Categoria) => {
  ...
  const updateForm = useCallback((key: string, value: any) => setForm((prev) => ({ ...prev, [key]: value })), []);
  ...
  const refreshAll = useCallback(async () => {
    ...
    setProspects((p.data || []).map(mapProspect));
    setSellers((s.data || []).map(mapSeller));
    setCupos((c.data || []).map(mapCupo));
  }, []);
  ...
  const byPlan = (arr: Seller[], plan: PlanType) => arr.filter((s) => s.tipo === plan);
  ...
  const monthlyBreakdown = useMemo<MonthlyRow[]>(() => {
    return MONTHS_SHORT.map((name, mi) => {
      ...
      r.total = PLAN_TYPES.reduce((sum, p) => sum + r[p], 0);
      return r;
    });
  }, [revenueSellers]);

  const ytdRev = useMemo(() => monthlyBreakdown.slice(0, CURRENT_MONTH + 1).reduce((s, m) => s + m.total, 0), [
    monthlyBreakdown,
  ]);
  ...
  const kpi = useMemo<KPI>(() => {
    ...
    const pipe = prospects.filter((p) => (ACTIVE_STAGES as readonly string[]).includes(p.st)).length;
    ...
    const planCounts = { Full: 0, Premium: 0, Basico: 0 } as Record<PlanType, number>;
    const planRevs = { Full: 0, Premium: 0, Basico: 0 } as Record<PlanType, number>;
  ...
  const planRevDist = useMemo(() => {
    return PLAN_TYPES.map((p) => ({ name: p, value: kpi.planRevs[p] || 0, fill: PLAN_COLORS[p] })).filter(
      (d) => d.value > 0
    );
  }, [kpi.planRevs]);
  ...
  const histogramData = useMemo<MonthlyRow[]>(() => {
  ...
  const groupedByCat = useMemo(() => {
    return CATEGORIAS.map((cat) => {
      ...
      const planBreakdown: Record<PlanType, { count: number; sellers: Seller[] }> = {
  ...
  const advance = (p: Prospect, ns: Stage) => {
    if (ns === 'Cerrados') {
  ...
  const saveMonthCharge = () => {
    ...
    const newD: Record<string, number> = { ...(s.customDctos || {}) };
  ...
  const rf = (label: string, k: string, opts?: { type?: string; options?: string[]; w?: string }) => (
    <FormField
      label={label}
      value={String(form[k] ?? '')}
  ...
  const StackedBarCell = (planKey: PlanType, isFuture: boolean) => {
          ...
          <div className="fi" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div className="card" style={{ overflow: 'hidden' }}>
              ...
              <div style={{ maxHeight: 400, overflowY: 'auto' }}>
                {filt.map((p) => {
                  const si = (ACTIVE_STAGES as readonly string[]).indexOf(p.st);
                  const nextA =
                    si >= 0 && si < ACTIVE_STAGES.length - 1
                      ? (ACTIVE_STAGES[si + 1] as Stage)
                      : undefined;
            ...
            <div className="card" style={{ padding: 18 }}>
              ...
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={histogramData}>
                  ...
                  <YAxis tick={{ fill: C.textMuted, fontSize: 9 }} axisLine={false} tickLine={false} tickFormatter={(v) => fmt(Number(v))} />
                  <Tooltip
                    contentStyle={{ background: C.bgCard, border: '1px solid ' + C.border, borderRadius: 10, fontSize: 12 }}
                    formatter={(v, name) => [fmtFull(Number(v)), String(name ?? '')]}
                  />
                  <Legend />
                  {PLAN_TYPES.map((plan) => (
              ...
              <div className="card" style={{ padding: 18 }}>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={revByCategory}>
                    ...
                    <YAxis tick={{ fill: C.textMuted, fontSize: 9 }} axisLine={false} tickLine={false} tickFormatter={(v) => fmt(Number(v))} />
                    <Tooltip contentStyle={{ background: C.bgCard, border: '1px solid ' + C.border, borderRadius: 10, fontSize: 12 }} formatter={(v) => fmtFull(Number(v))} />
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, minWidth: 900 }}>
                  <thead>
                    <tr style={{ background: C.bgAlt, borderBottom: '2px solid ' + C.border }}>
                      <th style={{ padding: '8px 14px', textAlign: 'left', fontWeight: 700, fontSize: 10, color: C.textMuted }}>
                        Plan
                      </th>
                      {MONTHS_SHORT.map((m) => (
                  ...
                  <tbody>
                    {PLAN_TYPES.map((plan) => {
                      ...
                      const totalPlan = monthlyBreakdown.reduce((s, m) => s + (m[plan] || 0), 0);
                      ...
                      if (isExpanded) {
                        PLAN_TYPES.forEach((plan) => {
                          ...
                          ps.forEach((s) => {
                            let yt = 0;
                            rows.push(
                              <tr key={s.sid + '-' + plan} className="row-hover" style={{ borderBottom: '1px solid ' + C.borderLight }}>
                                ...
                                <td style={{ padding: '7px 8px' }}>
                                  <Pill color={pc}>{plan}</Pill>
                                </td>
                                <td style={{ padding: '7px 8px', fontWeight: 600 }}>{fmt(s.tarifa)}</td>
                                <td style={{ padding: '7px 8px', color: s.dcto > 0 ? C.purple : C.textMuted }}>
                                  {s.dcto > 0 ? s.dcto + 'm' : '-'}
                                </td>
