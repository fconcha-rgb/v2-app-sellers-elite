import {
  ...
  export default function App() {
    ...
    const groupedByCat = useMemo<GroupedByCat[]>(() => {
  ...
  {PLAN_TYPES.map(p => (
    ...
    >
      ...
      <span>{p}</span>
              ...
              {/* ═══ COLLAPSIBLE DETAIL TABLE (nueva versión) ═══ */}
              <div className="card" style={{ overflow: 'hidden' }}>
                <div
                  style={{
                    padding: '12px 16px',
                            ...
                            <td colSpan={7} style={{ padding: '8px 8px', fontWeight: 700, fontSize: 12, color: C.text }}>
                              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                                ...
                                {PLAN_TYPES.map((p) => {
                                  ...
                                  if (!cnt) return null;
                                  return (
                                    <span key={p} style={{ fontSize: 9, padding: '1px 6px', borderRadius: 10, background: PLAN_COLORS[p] + '20', color: PLAN_COLORS[p], fontWeight: 700 }}>
                                      {cnt + ' ' + p}
                                    </span>
                                  );
                                })}
